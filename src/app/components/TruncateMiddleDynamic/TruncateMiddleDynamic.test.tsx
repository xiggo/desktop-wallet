...
describe("TruncateMiddleDynamic", () => {
	...
	const Component = ({ value, offset }: { value: string; offset?: number }) => {
		const referenceElement = useRef(parentElement);

		return <TruncateMiddleDynamic parentRef={referenceElement} value={value} offset={offset} tooltipDarkTheme />;
	};
	...
	it("should render with tooltip in the dark mode", () => {
		...
		const { rerender } = render(<Component value={valueToTruncate} offset={5} />);

		expect(screen.getByText(valueToTruncate)).toBeInTheDocument();

		rerender(<Component value={valueToTruncate} offset={5} />);
	...
	it("should truncate the value if it does not fit the given container", () => {
		...
		expect(screen.getByText("Lorem ipsum dolor sit am…ctetur adipisicing elit.")).toBeInTheDocument();
	});
