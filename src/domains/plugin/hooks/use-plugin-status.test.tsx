...
describe("#usePluginStatus", () => {
	it("should return `not installed` status", () => {
		...
		const status = result.current.renderPluginStatus({ isEnabled: false, isInstalled: false });
	...
	it("should render `disabled` status", () => {
		const { result } = renderHook(() => usePluginStatus());
		const status = result.current.renderPluginStatus({ isEnabled: false, isInstalled: true });
	...
	it("should render `enabled` status", () => {
		const { result } = renderHook(() => usePluginStatus());
		const status = result.current.renderPluginStatus({ isEnabled: true, isInstalled: true });
